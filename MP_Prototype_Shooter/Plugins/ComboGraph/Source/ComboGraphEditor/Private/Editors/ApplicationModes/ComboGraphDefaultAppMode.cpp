...
FComboGraphDefaultAppMode::FComboGraphDefaultAppMode(const TSharedRef<FComboGraphAssetEditor>& InAssetEditor)
	: FApplicationMode(FComboGraphEditorModes::ComboGraphDefaultModeID)
...
void FComboGraphDefaultAppMode::RegisterTabFactories(const TSharedPtr<FTabManager> InTabManager)
{
...
TSharedPtr<FTabManager::FLayout> FComboGraphDefaultAppMode::GenerateTabLayout(const TSharedRef<FAssetEditorToolkit>& InAssetEditor)
{
	return FTabManager::NewLayout("Standalone_ComboGraphDefaultEditMode_Layout_v1.0.0")
	->AddArea
	(
		...
		(
			...
			(
				...
				(
					...
					->AddTab(FPersonaTabs::AdvancedPreviewSceneSettingsID, ETabState::OpenedTab)
					->SetForegroundTab(FComboGraphEditorTabs::AssetBrowserID)
				)
			)
		)
	);
...
TSharedRef<FWorkflowTabFactory> FComboGraphDefaultAppMode::CreateComboGraphViewportTabFactory(const TSharedRef<FAssetEditorToolkit>& InHostingApp, const TSharedRef<FComboGraphAssetEditor>& InAssetEditor) const
{
	return MakeShareable(new FComboGraphViewportTabFactory(InHostingApp, InAssetEditor));
}

TSharedRef<FWorkflowTabFactory> FComboGraphDefaultAppMode::CreatePropertyDetailsTabFactory(const TSharedRef<FAssetEditorToolkit>& InHostingApp, const TSharedRef<FComboGraphAssetEditor>& InAssetEditor) const
{
