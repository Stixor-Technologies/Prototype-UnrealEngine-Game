...
class COMBOGRAPHEDITOR_API FComboGraphDragDropAction : public FGraphEditorDragDropAction
{
public:
	DRAG_DROP_OPERATOR_TYPE(FACEGraphDragAction, FGraphEditorDragDropAction)

	FComboGraphDragDropAction(const TSharedRef<SGraphPanel>& GraphPanel, TArray<FGraphPinHandle>& DraggedPins);
	static TSharedRef<FComboGraphDragDropAction> New(const TSharedRef<SGraphPanel>& GraphPanel, TArray<FGraphPinHandle>& InStartingPins);
...
private:
	...
	void AppendConnectionResponsesForHoveredNode(const UComboGraphEdNode* TargetNode, TArray<FPinConnectionResponse>& OutResponses);
